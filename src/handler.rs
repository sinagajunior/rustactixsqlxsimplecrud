...
pub async fn note_list_handler(
    ...
    if query_result.is_err() {
        let message = "Something bad happened while fetching all noted items";
...
async fn create_note_handler(
    ...
    match query_result {
        ...
        Err(e) => {
            ...
            .json(serde_json::json!({"status":"fail","message": "Note with that title already exits"}));
            }
            return HttpResponse::InternalServerError()
...
async fn get_note_handler(
    ...
    match query_result {
        ...
        Err(_) => {
            let message = format!("Note with ID: {} note found", note_id);
